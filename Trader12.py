"""
Trader12 — PEPPER: passive spread-scalp after full. Accumulate to 80 like baseline,
then once at 80 post a passive ask at best_ask-1 (size 10). When position dips
below 80, re-buy passively at best_bid+1 until full again.
OSMIUM: baseline MM.
Hypothesis: memory says dynamic rebalancing lost -11k because it CROSSED spread.
MAKING the spread should flip the sign — each round trip earns spread-1 instead
of losing it. Position floor=70 preserves most of the trend PnL.
"""
import json
...
class Trader:
    ...
    def trade_pepper(self, product, order_depth, position, limit):
        ...
        best_ask = min(order_depth.sell_orders.keys())
        if position < limit:
            # Still accumulating: take aggressively
            ...
            for ask_price in sorted(order_depth.sell_orders.keys()):
                ...
                if can_buy <= 0:
                    break
            return orders
        # position == limit: scalp the spread
        # Post a passive ask slightly inside the book
        sell_qty = min(SCALP_SIZE, position - POSITION_FLOOR)
        if sell_qty > 0:
            orders.append(Order(product, best_ask - 1, -sell_qty))
        # Always be ready to rebuild with a passive bid
