"""
Trader9 — PEPPER: passive-only accumulation (never cross the spread).
OSMIUM: baseline MM.
Hypothesis: baseline pays ~640 crossing the 12-13 spread on the initial fill. If we
post passive bids at best_bid+1, we save that cost. Risk: slower fills = less trend
exposure. Net should still be positive because the trend is persistent.
"""
import json
...
class Trader:
    ...
    def trade_pepper(self, product, order_depth, position, limit):
        ...
        if can_buy <= 0:
            return orders
        best_bid = max(order_depth.buy_orders.keys())
        # Post passive bid one tick above best_bid; never cross
        orders.append(Order(product, best_bid + 1, can_buy))
        return orders
