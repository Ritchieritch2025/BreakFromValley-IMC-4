"""
Trader5 — OSMIUM: widen passive quotes when spread is wide (>=18).
PEPPER: baseline buy-and-hold.
Hypothesis: in ~22% of ticks the spread >= 18. Quoting at FV±2 (instead of FV±1)
still joins inside the wide spread AND earns an extra tick per round trip.
"""
import json
    ...
    def trade_osmium(self, product, order_depth, position, limit):
        ...
        if position > 0 and order_depth.buy_orders.get(fair_int, 0) > 0:
            ...
            if qty > 0:
                orders.append(Order(product, fair_int, qty))
                buy_vol += qty
        # Widen quote distance in wide spreads
        offset = 2 if spread >= 18 else 1
        buy_price = min(fair_int - offset, best_bid + 1)
        sell_price = max(fair_int + offset, best_ask - 1)
        skew = int((position / limit) * 3)
        buy_price -= skew
        sell_price -= skew
        buy_price = min(buy_price, fair_int - offset)
        sell_price = max(sell_price, fair_int + offset)
