"""
Trader7 — OSMIUM: conservative take — only take when ask <= FV-1 (sell when bid >= FV+1).
PEPPER: baseline buy-and-hold.
Hypothesis: baseline takes anything priced at FV-0.5 (half-tick edge). Demanding a
full tick of edge should avoid buying at just-below-fair prints that then drift down.
"""
import json
...
class Trader:
    ...
    def trade_osmium(self, product, order_depth, position, limit):
        ...
        buy_vol = 0
        sell_vol = 0
        # Require ask <= FV - 1 (full tick of edge)
