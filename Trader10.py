"""
Trader10 — PEPPER: split entry. Take 40 aggressively at t=0, accumulate remaining
40 passively at best_bid+1.
OSMIUM: baseline MM.
Hypothesis: halves the initial spread-cost drawdown vs. full-aggressive while still
getting half-position on the trend immediately. Passive half recoups more later.
"""
import json
from datamodel import Order, TradingState

AGGRESSIVE_TARGET = 40
...
class Trader:
    ...
    def trade_pepper(self, product, order_depth, position, limit):
        ...
        if not order_depth.sell_orders or not order_depth.buy_orders:
            return orders
        # Aggressive leg: take until position reaches AGGRESSIVE_TARGET
        need_aggressive = max(0, AGGRESSIVE_TARGET - position)
        taken = 0
        if need_aggressive > 0:
            for ask_price in sorted(order_depth.sell_orders.keys()):
                ask_qty = abs(order_depth.sell_orders[ask_price])
                qty = min(ask_qty, need_aggressive - taken)
                if qty <= 0:
                    break
                orders.append(Order(product, ask_price, qty))
                taken += qty
                if taken >= need_aggressive:
                    break
        # Passive leg: post remaining at best_bid+1
        effective_pos = position + taken
        rem_passive = limit - effective_pos
        if rem_passive > 0:
            best_bid = max(order_depth.buy_orders.keys())
            orders.append(Order(product, best_bid + 1, rem_passive))
        return orders
