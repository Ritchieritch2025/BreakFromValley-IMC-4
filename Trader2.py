"""
Trader2 — OSMIUM: skew=0 (no inventory tilt).
PEPPER: baseline buy-and-hold.
Hypothesis: with strong mean reversion (-0.495 autocorr), skewing away from position
may be hurting — a large position is MORE likely to revert profitably.
"""
import json
from datamodel import Order, TradingState

SKEW_MULT = 0  # <<< this is the knob
...
class Trader:
    ...
    def trade_osmium(self, product, order_depth, position, limit):
        ...
        skew = int((position / limit) * SKEW_MULT)
        buy_price -= skew
        sell_price -= skew
